...
optimized:
	CFLAGS="${CFLAGS_RCB_OPT} -s" $(RCB) $(RCBFLAGS) ${MAINFILE} $(LINKLIBS)
	strip --remove-section .comment ${OUTFILE}

optimized-aggressive:
	CFLAGS="${CFLAGS_RCB_OPT_AGGRESSIVE} -s" $(RCB) $(RCBFLAGS) ${MAINFILE} $(LINKLIBS)
	strip --remove-section .comment ${OUTFILE}

odebug:
	CFLAGS="${CFLAGS_RCB_OPT} -g" $(RCB) $(RCBFLAGS) ${MAINFILE} $(LINKLIBS)
	debug-stripper.sh $(OUTFILE)

debug:
	CFLAGS="${CFLAGS_RCB_DBG}" $(RCB) $(RCBFLAGS) ${MAINFILE} $(LINKLIBS)
